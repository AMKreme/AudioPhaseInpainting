...
from debug import phasecut_inpainting, PhasecutTracker
from algorithms import random_phase_inpainting, griffin_lim_inpainting, \
    GLTracker
...
class PhasecutVarMissRatioExp(PhasecutExperiment):
    ...
    def run_task(self, task_id):
        ...
        if solver == 'RPI':
            ...
            # Build tracker
            tracker = PhasecutTracker(x_ref=self.x_ref, B=B,
                                      stft=stft, istft=istft)
    ...
    def collect_results(self):
        ...
        track_residue = [[None for _ in range(n_miss)] for _ in range(
            n_solvers)]
        ...
        for i_task in range(self.get_n_tasks()):
            ...
            if Path(filename).exists():
                ...
                track_residue[i_solver][i_ratio] = data['track_residue']
    ...
    def plot_results(self):
        ...
        plt.ylabel('Error (dB)')
        # plt.title('')
        plt.savefig(str(self.get_figure_folder() / 'err_ratio.pdf'))
        plt.show()
        ...
        for i_solver in range(len(self.solvers)):
            ...
            plt.ylabel('Error')
            plt.title(self.solvers[i_solver])
            plt.legend()
            plt.subplot(122)
            if self.solvers[i_solver] == 'GLI':
                continue
            for i_ratio in range(self.missing_ratios.size):
                plt.plot(
                    res['track_iter'][i_solver][i_ratio],
                    res['track_residue'][i_solver][i_ratio],
                    label='{:.0%} missing'.format(
                        self.missing_ratios[i_ratio]))
            plt.xlabel('Iterations')
            plt.ylabel('Residue error')
            plt.title(self.solvers[i_solver])
            plt.legend()
            plt.savefig(str(self.get_figure_folder() / 'err_iter_{}.pdf')
...
exp_instances = {
    'debug0': {'win_len': 16,
                  'hop': 8,
                  'nb_bins': 32,
                  'missing_ratios': np.arange(0, 0.5, 0.2),
                  'phasecut_n_iter': 100,
                  'phasecut_nu': 1e-2,
                  'gli_n_iter': 100},
    # 'large_exp': {'win_len': 16,
    #               'hop': 8,
    #               'nb_bins': 32,
    #               'missing_ratios': np.arange(0, 1, 0.1),
    #               'phasecut_n_iter': 6000,
    #               'phasecut_nu': 1e-2,
    #               'gli_n_iter': 6000},
    # 'hop2_bins2_exp': {'win_len': 16,
    #               'hop': 4,
    #               'nb_bins': 64,
    #               'missing_ratios': np.arange(0, 1, 0.1),
    #               'phasecut_n_iter': 6000,
...
if __name__ == '__main__':
    ...
    # PhasecutExperiment.load_exp('debug0').collect_results()
    # PhasecutExperiment.load_exp('debug0').plot_results()
    run_exp('debug0')
    # run_exp('hop2_exp')
    # run_exp('bins2_exp')
